import { BriefcaseBusiness, BriefcaseIcon, Globe, Mail, MapPin, Phone, User } from 'lucide-react'
import React from 'react'

const PersonalDataForm = ({ data, onChange, removeBackground, setRemoveBackground }) => {
    ...
    const fields = [
        ...
        { key: 'linkedin', label: 'LinkedIn Profile', icon: BriefcaseIcon, type: 'url' },
                ...
                <label>
                    {data.image ? (
                        <img src={typeof data.image === 'string' ? data.image : URL.createObjectURL(data.image)} alt="user-image"
                            className='w-20 h-20 rounded-full object-cover mt-2 ring ring-green-500 hover:opacity-80 cursor-pointer' />
                    ) : (
